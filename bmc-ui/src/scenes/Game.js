...
export class Game extends Scene {
  ...
  create() {
    // Get game mode and user token from registry
    this.gameMode = this.registry.get("gameMode") || "legacy";
    this.userToken = this.registry.get("userToken");
    ...
    // Create characters based on game mode
    if (this.gameMode === "business") {
      this.createBusinessExperts(map, layers);
    } else {
    ...
    // Initialize the dialogue manager with game mode and user token
  ...
  createBusinessExperts(map, layers) {
    const businessExpertConfigs = [
      {
    ...
    businessExpertConfigs.forEach((config, index) => {
      // Use philosopher spawn points for business experts
      const spawnPoints = [
        "Socrates",
        "Plato",
        "Aristotle",
      ...
      const spawnPointName = spawnPoints[index] || "Socrates";
      const spawnPoint = map.findObject(
        "Objects",
        (obj) => obj.name === spawnPointName
      );
      ...
      this[config.id] = new Character(this, {
        ...
        atlas: config.atlas, // Reuse philosopher sprites for now
        atlasId: config.atlas, // Use the atlas name for frame references
    ...
    // Make all expert labels visible initially
    this.togglePhilosopherLabels(true);

    // Add collisions between experts
